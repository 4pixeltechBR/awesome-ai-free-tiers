...
# Compilação e Validação Fina: 19 de setembro de 2026 (v15)
# Histórico: 27/05 → 15/07 → 23/07 → 16/08 → 19/08 → 25/08 (v8) → 06/09 (v9) → 17/09 (v10) → 17/09 (v11) → 17/09 (v12) → 17/09 (v13) → 18/09 (v14) → 19/09/2026 (v15 atual)
# Fontes: Catálogo Operacional RUNTIME Real + Absorção Auditada OmniRoute v3.8.51 + Auditoria Global v15
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
[NÍVEL 2 — High Throughput, Modelos Abertos & Multimodalidade (Zero Cost)]
   │
   ├─► 5. Inception Labs (mercury-chat / mercury-coder) 💎
   │       Cota: 100 Milhões de tokens grátis no signup (60 RPM, 5 concorrência, ultra-rápido)
   │
   ├─► 6. Reka AI (reka-flash / reka-core / reka-edge) 💎
   │       Cota: $10 USD/mês em créditos recorrentes + 3h vídeo (Multimodal frontier, sem cartão)
   │
   ├─► 7. SiliconFlow Free Tier (Qwen2.5-7B / Coder / Qwen2.5-VL / DeepSeek-R1-Distill) 🆕
   │       Cota: 1.000 RPM / 40.000 TPM (Modelos free ilimitados + 20M tokens no cadastro)
   │
   ├─► 8. OpenRouter Free Tier (nex-agi/nex-n2.5-pro / inclusionai/ling-3.0-flash-vl)
   │       Cota: 20 RPM / 200 RPD por modelo (24 modelos ativos com Visão e 262K ctx)
   │
   ├─► 9. Maritaca AI — MariTalk (sabia-4 / sabia-4-thinking / sabiazinho-4) 🇧🇷
   │       Cota: 50 RPM / 500.000 TPM (A campeã em português, sem cartão)
   │
   ├─► 10. Bytez (llama-3.3-70b / qwen-2.5-72b) 💎
   │       Cota: $1.00 USD renovado a cada 4 semanas (sem cartão)
   │
   ├─► 11. Mistral AI (mistral-small-latest / codestral-latest)
   │       Cota: 60 RPM / 50K TPM pool / 4M tokens por mês (Sem cartão)
   │
   └─► 12. Pollinations.ai (openai / deepseek / qwen-coder / flux) 🆕
           Cota: 60 RPM texto / 30 RPM imagem (100% free perpétuo, endpoints OpenAI/REST)

[NÍVEL 3 — Especialistas em RAG, Rerank, Código, Agentes & Cloud Serverless]
   │
   ├─► 13. Morph Labs Fast Apply (morph-fast-apply) 💎
   │       Cota: 250.000 créditos/mês ($0) + ~200 diffs/mês (Acelerador instantâneo para agentes de código)
   │
   ├─► 14. Cloudflare Workers AI (llama-3.3-70b / deepseek-r1-distill / flux-1-schnell) ☁️
   │       Cota: 10.000 Neurons/dia (~100k-500k tokens diários gratuitos perpétuos sem cartão)
   │
   ├─► 15. SambaNova Cloud (Llama-3.3-70B / DeepSeek-R1 em chips SN40L RDU) ⚡
   │       Cota: 30 RPM / 6.000 TPM (Inferência com centenas de tokens/segundo)
   │
   ├─► 16. GitHub Models (Claude 3.5 Sonnet / GPT-4o / DeepSeek-R1 via Azure AI)
   │       Cota: 15 RPM / 150 RPD (Acesso gratuito a modelos proprietários fechados)
   │
   ├─► 17. Cohere Developer Platform (command-r-plus / rerank-v3.5 / embed-multilingual-v3) 🆕
   │       Cota: 1.000 reqs/mês, 20 RPM chat, 10 RPM rerank, 100 RPM embed (Líder em RAG)
   │
   ├─► 18. Nomic AI & Mixedbread AI (nomic-embed-text-v1.5 / mxbai-rerank-large-v1) 🎯
   │       Cota: Milhares de embeddings e reranks gratuitos sem cartão
   │
   ├─► 19. AwanLLM Lite (Meta-Llama-3.1-8B / Awanllm-Llama-3-8B-Cumulus) 🆕
   │       Cota: 20 RPM / 200 RPD pequenos ("Unlimited tokens", sem cartão, zero-refusal)
   │
   └─► 20. Modal Labs ($30/mês compute credits) 🖥️
           Cota: $30 USD/mês em contêineres e GPUs serverless para vLLM/Ollama (requer cartão)

[NÍVEL 4 — Onboarding Credits Elevados & Pay-per-Use Ultra-Econômico]
   │
   ├─► 21. Nebius Token Factory (Llama 3.1 405B / DeepSeek V3/R1) 🆕
   │       Cota: $400+ créditos no AI Builder Program (Auto-scaling dinâmico de 60+ RPM)
   │
   ├─► 22. Novita AI (meta-llama/llama-3.1-70b / flux.1-schnell) 🆕
   │       Cota: $10 a $100 em trial credits de sandbox (20 IPM imagem / 60 RPM chat)
   │
   ├─► 23. DeepSeek API Direta (deepseek-chat [V3] / deepseek-reasoner [R1])
   ...
   │
   ├─► 24. Kimi / Moonshot AI (kimi-k3 / kimi-k2.7-code via api.moonshot.ai)
   │       Custo: $0.95 a $3.00 por milhão (Tier 0 ativado com recarga única de $1)
   │
   └─► 25. DeepInfra (gpt-oss-120b)
...
[ROTA ANÔNIMA & SEM CHAVE: ZERO CADASTRO, ZERO RETENÇÃO] 🕵️
   │
   ├─► 33. LLM7.io (api.llm7.io/v1) — 2 req/s, 20 RPM, 100 req/hr livres sem login 💎
   ├─► 34. AI Horde (oai.aihorde.net/v1) — Chave pública `0000000000` em cluster comunitário
   ├─► 35. UncloseAI (uncloseai.com) — Endpoint mock/proxy para testes de integração
   └─► 36. DuckDuckGo AI (duckduckgo.com/duckchat) — Interface anônima com Claude/GPT-4o mini
...
### Validação Executada em 19/09/2026 (Consolidado v15)

| Provedor Auditado | Endpoint Validado | Modelos no Catálogo | Status de Resposta | Diagnóstico / Achados Operacionais (19/09) |
| :--- | :--- | :---: | :---: | :--- |
